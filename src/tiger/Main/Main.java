...
public class Main {
	...
	static java.io.PrintStream outir;
	static java.io.PrintStream outerr;
	...
	static tiger.Assem.InstrList codegen(tiger.Frame.Frame f,
		...
		tiger.Assem.InstrList first = null, last = null, it;
		for (tiger.Tree.StmList s = stms; s != null; s = s.tail) {
			...
			// debuging
			// for (it = i; it != null; it = it.tail)
			// System.out.print(it.head.assem);
	...
	public static void run(String args[]) throws java.io.IOException {
		System.out.println("Beging Compiling...");
		String testTxt = args[0];



		/*Syntactic (packing in parse.Runable)*/
		...
		//new tiger.FindEscape.FindEscape(parseExp);

		//Translate Phase
		tiger.Translate.Frag frags = semant.transProg(translate.mainFrame,
				parseExp);

		if (parseRun.errorMsg.anyErrors)
			return;
		outir = new java.io.PrintStream(new java.io.FileOutputStream(testTxt
				+ ".ir"));
		for (tiger.Translate.Frag f = frags; f != null; f = f.next)
			if (f instanceof tiger.Translate.ProcFrag)
				emitProc(outir, (tiger.Translate.ProcFrag) f);
			else if (f instanceof tiger.Translate.DataFrag) {
				outir.print(((tiger.Translate.DataFrag) f).data);
			}

		System.out.println("Compiler successful,"+testTxt+".ir  have been gened!");
	}
/*
