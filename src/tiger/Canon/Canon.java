...
public class Canon {
	...
	static boolean commute(Stm a, AbsExp b) {
	...
	static Stm do_stm(MOVE s) {
		if (s.dst instanceof TEMP && s.src instanceof CALL)
			...
			return do_stm(new SEQ(((ESEQ) s.dst).stm, new MOVE(
					((ESEQ) s.dst).exp, s.src)));
		else
	...
	static ESEQ reorder_exp(AbsExp e) {
	...
	static StmExpList reorder(ExpList exps) {
		if (exps == null)
			...
			if (a instanceof CALL) {
				Temp t = new Temp();
				AbsExp e = new ESEQ(new MOVE(new TEMP(t), a), new TEMP(t));
				return reorder(new ExpList(e, exps.tail));
			} else {
				ESEQ aa = do_exp(a);
				StmExpList bb = reorder(exps.tail);
				if (commute(bb.stm, aa.exp))
					return new StmExpList(seq(aa.stm, bb.stm), new ExpList(
							aa.exp, bb.exps));
				else {
					Temp t = new Temp();
					return new StmExpList(seq(aa.stm, seq(new MOVE(new TEMP(t),
							aa.exp), bb.stm)),
